...
2. Scan project skills at `.a-team/skills/`
...
5. **Ask for scope:**
   ...
   - `project` → `.a-team/skills/<name>/SKILL.md`
...
User: yes
Assistant: ✓ Copied 'error-handler' to .a-team/skills/
...
PROJECT_SKILLS_DIR=".a-team/skills"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.a-team/skills/) ==="
if [ -d ".a-team/skills" ]; then
  PROJECT_COUNT=$(find ".a-team/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".a-team/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
- **Project-level** (.a-team/skills/) - Only for this project
...
**Version Control**: Project-level skills (.a-team/skills/) are committed with your code, so the whole team benefits.
...
✓ Project skills directory exists: .a-team/skills/
