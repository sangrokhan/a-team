...
**You don't need to learn any commands!** A Team enhances Codex CLI with intelligent behaviors that activate automatically.
...
If you haven't configured A Team yet:
...
1. Reads token tracking from `~/.a-team/state/token-tracking.jsonl`
2. Reads session history from `.a-team/state/session-history.json`
...
TOKEN_FILE="$HOME/.a-team/state/token-tracking.jsonl"
SESSION_FILE=".a-team/state/session-history.json"
CONFIG_FILE="$HOME/.codex/.a-team-config.json"

echo "Analyzing A Team Usage..."
echo ""
...
1. Ensure ~/.a-team/state/ directory exists
2. Run any A Team command to start tracking

Tip: Run /omc-setup to configure A Team properly.
```
