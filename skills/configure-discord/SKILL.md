...
Set up Discord notifications so A Team can ping you when sessions end, need input, or complete background tasks.

## How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `~/.codex/.a-team-config.json`.

## Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="$HOME/.codex/.a-team-config.json"
...
**Options:**
1. **A Team (default)** - Display as "A Team"
...
Build the notifications object with the collected values and merge into `.a-team-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}A Team test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
Config saved to: ~/.codex/.a-team-config.json

You can also set these via environment variables:
  A_TEAM_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  A_TEAM_DISCORD_MENTION=<@1465264645320474637>
...
**Webhook method:**
```bash
export A_TEAM_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export A_TEAM_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export A_TEAM_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export A_TEAM_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export A_TEAM_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

Env vars are auto-detected by the notification system without needing `.a-team-config.json`.
