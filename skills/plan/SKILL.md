...
8. On user approval: **MUST** invoke `/ralph` with the approved plan path from `.a-team/plans/` as context. Do NOT implement directly. Do NOT edit source code files in the planning agent. The ralph skill handles execution via ultrawork parallel agents.

### Review Mode (`--review`)

1. Read plan file from `.a-team/plans/`
...
Plans are saved to `.a-team/plans/`. Drafts go to `.a-team/drafts/`.
</Steps>
...
- If ToolSearch finds no MCP tools or Codex is unavailable, fall back to equivalent A Team prompt agents -- never block on external tools
