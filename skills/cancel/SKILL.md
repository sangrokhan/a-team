---
name: cancel
description: Cancel any active A Team mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline, team)
---

# Cancel Skill

Intelligent cancellation that detects and cancels the active A Team mode.

**The cancel skill is the standard way to complete and exit any A Team mode.**
...
- By default the command inspects the current session via `state_list_active` and `state_get_status`, navigating `.a-team/state/sessions/{sessionId}/…` to discover which mode is active.
- When a session id is provided or already known, that session-scoped path is authoritative. Legacy files in `.a-team/state/*.json` are consulted only as a compatibility fallback if the session id is missing or empty.
- Swarm is a shared SQLite/marker mode (`.a-team/state/swarm.db` / `.a-team/state/swarm-active.marker`) and is not session-scoped.
...
1. `state_list_active` enumerates `.a-team/state/sessions/{sessionId}/…` to find every known session.
2. `state_clear` runs once per session to drop that session’s files.
3. A global `state_clear` without `session_id` removes legacy files under `.a-team/state/*.json`, `.a-team/state/swarm*.db`, and compatibility artifacts (see list).
4. Team artifacts (`.a-team/state/team/*/`, tmux sessions matching `a-team-team-*`) are best-effort cleared as part of the legacy fallback.

Every `state_clear` command honors the `session_id` argument, so even force mode still uses the session-aware paths first before deleting legacy files.

Legacy compatibility list (removed only under `--force`/`--all`):
- `.a-team/state/autopilot-state.json`
- `.a-team/state/ralph-state.json`
- `.a-team/state/ralph-plan-state.json`
- `.a-team/state/ralph-verification.json`
- `.a-team/state/ultrawork-state.json`
- `.a-team/state/ecomode-state.json`
- `.a-team/state/ultraqa-state.json`
- `.a-team/state/swarm.db`
- `.a-team/state/swarm.db-wal`
- `.a-team/state/swarm.db-shm`
- `.a-team/state/swarm-active.marker`
- `.a-team/state/swarm-tasks.db`
- `.a-team/state/ultrapilot-state.json`
- `.a-team/state/ultrapilot-ownership.json`
- `.a-team/state/pipeline-state.json`
- `.a-team/state/plan-consensus.json`
- `.a-team/state/ralplan-state.json`
- `.a-team/state/boulder.json`
- `.a-team/state/hud-state.json`
- `.a-team/state/subagent-tracking.json`
- `.a-team/state/subagent-tracker.lock`
- `.a-team/state/rate-limit-daemon.pid`
- `.a-team/state/rate-limit-daemon.log`
- `.a-team/state/checkpoints/` (directory)
- `.a-team/state/sessions/` (empty directory cleanup after clearing sessions)
...
1. Call `state_list_active` to enumerate `.a-team/state/sessions/{sessionId}/…` and discover every active session.
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.a-team/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected by checking for config files in `.a-team/state/team/`:

```bash
# Check for active teams
ls .a-team/state/team/*/config.json 2>/dev/null
```
...
```
For each team found in .a-team/state/team/:
  1. Read config.json to get team_name and workers list
  2. For each worker:
     a. Write shutdown inbox to .a-team/state/team/{name}/workers/{worker}/inbox.md
...
After graceful pass:
  ...
  2. Destroy the tmux session: tmux kill-session -t a-team-team-{name}
```

**Cleanup:**
```
  1. Strip AGENTS.md team worker overlay (<!-- A Team:TEAM:WORKER:START/END -->)
  2. Remove team state directory: rm -rf .a-team/state/team/{name}/
...
1. Check `.a-team/state/team/*/config.json` for active teams
...
5. Destroy tmux session: `tmux kill-session -t a-team-team-{name}`
6. Strip AGENTS.md overlay
7. Remove state: `rm -rf .a-team/state/team/{name}/`
...
if [[ -f .a-team/state/autopilot-state.json ]]; then
  # Clean up ralph if active
  if [[ -f .a-team/state/ralph-state.json ]]; then
    RALPH_STATE=$(cat .a-team/state/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .a-team/state/ultrawork-state.json ]]; then
      rm -f .a-team/state/ultrawork-state.json
      echo "Cleaned up: ultrawork (linked to ralph)"
    fi

    # Clean ralph
    rm -f .a-team/state/ralph-state.json
    rm -f .a-team/state/ralph-verification.json
    echo "Cleaned up: ralph"
  fi

  # Clean up ultraqa if active
  if [[ -f .a-team/state/ultraqa-state.json ]]; then
    rm -f .a-team/state/ultraqa-state.json
    echo "Cleaned up: ultraqa"
  fi

  # Mark autopilot inactive but preserve state
  CURRENT_STATE=$(cat .a-team/state/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .a-team/state/autopilot-state.json
...
if [[ -f .a-team/state/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .a-team/state/ultrawork-state.json ]]; then
    UW_STATE=$(cat .a-team/state/ultrawork-state.json)
  ...
  # Clean ralph state
  rm -f .a-team/state/ralph-state.json
  rm -f .a-team/state/ralph-plan-state.json
  rm -f .a-team/state/ralph-verification.json
...
```bash
if [[ -f .a-team/state/ultrawork-state.json ]]; then
...
```bash
echo "No active A Team modes detected."
echo ""
echo "Checked for:"
echo "  - Autopilot (.a-team/state/autopilot-state.json)"
echo "  - Ralph (.a-team/state/ralph-state.json)"
echo "  - Ultrawork (.a-team/state/ultrawork-state.json)"
echo "  - UltraQA (.a-team/state/ultraqa-state.json)"
echo ""
...
4. In force mode, iterate every active session, call `state_clear` per session, then run a global `state_clear` without `session_id` to drop legacy files (`.a-team/state/*.json`, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
5. Team artifacts (`.a-team/state/team/*/`, tmux sessions matching `a-team-team-*`) remain best-effort cleanup items invoked during the legacy/global pass.
...
| Force | "All A Team modes cleared. You are free to start fresh." |
| None | "No active A Team modes detected." |
...
- **Local-only**: Clears state files in `.a-team/state/` directory
...
1. **Kill all team tmux sessions**: `tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^a-team-team-'` and kill each
2. **Remove team state directories**: `rm -rf .a-team/state/team/*/`
3. **Strip AGENTS.md overlay**: Remove content between `<!-- A Team:TEAM:WORKER:START -->` and `<!-- A Team:TEAM:WORKER:END -->`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .a-team/state/team/                  # All team state
# Kill all a-team-team-* tmux sessions
tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^a-team-team-' | while read s; do tmux kill-session -t "$s" 2>/dev/null; done
```
